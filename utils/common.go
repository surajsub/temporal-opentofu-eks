...
func RunTFSGApplyCommand(directory, vpcid, vpcdir string) (string, error) {
	...
	output, err := cmd.CombinedOutput()
	if err != nil {
		return "", fmt.Errorf(SgWorkflow+"error running command %s %v: %v - output: %s", "terraform", "apply -json", err, string(output))
	}
	return string(output), nil
}
...
func RunTFEC2ApplyCommand(directory, subnetid, sgid string) (string, error) {
	...
	output, err := cmd.CombinedOutput()
	if err != nil {
		return "", fmt.Errorf(Ec2Workflow+"error running command %s %v: %v - output: %s", "terraform", "apply -json", err, string(output))
	}
	return string(output), nil
}
